...
## 실습 장비 소개 
EduPhysX 는 브레드보드를 기반으로 하여 납땜 과정 없이도 다양한 IoT 캡스톤 시나리오를 즉시 구현하실 수 있도록 설계된 모듈형 피지컬 컴퓨팅 실습 플랫폼입니다. 8-bit RISC(AVR 계열) 기반으로 구성되어 있어 기초 실습(입출력/타이머/통신)을 간결하게 진행할 수 있습니다.
...
또한 PC의 Python 프로그램에서 센서 데이터 수집/처리/제어 명령을 구성하고, 통신(예: Serial 등)으로 MCU에 전달하여 액추에이터를 동작시키는 형태로 전체 파이프라인을 구성할 수 있습니다.
