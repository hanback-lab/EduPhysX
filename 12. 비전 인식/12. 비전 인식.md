...
### 카메라 영상 출력 주요 코드 설명
> **cv2.VideoCapture(0)**
> - PC에 연결된 카메라 장치를 엽니다. 숫자 0은 첫 번째로 인식된 카메라를 의미합니다.

> **ret, frame = cam.read()**
> - 카메라로부터 한 프레임을 읽어옵니다. ret은 프레임 수신 성공 여부를 나타냅니다.

> **cv2.imshow('frame', frame)**
> - 읽어온 영상 프레임을 화면 창에 출력합니다.

> **cv2.waitKey(1)**
> - 키보드 입력을 감지하며, 영상이 연속적으로 갱신되도록 합니다.
...
> - OpenCV 기본 색상 형식인 BGR 이미지를 RGB 색상 공간으로 변환합니다.

> **cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)**
> - 컬러 영상을 흑백 영상으로 변환하여 밝기 정보만 남깁니다.
...
### ROI 주요 코드 설명
> **frame[y1:y2, x1:x2]**
> - 영상에서 관심 영역(ROI)을 슬라이싱 방식으로 잘라냅니다.

> **cv2.rectangle()**
> - ROI 영역을 시각적으로 확인하기 위해 사각형을 그립니다.
...
### 손 인식 주요 코드 설명 
> **hands.process(image)**
> - MediaPipe Hands 모델에 현재 프레임을 입력하여 손 인식을 수행합니다. 프레임 내에 손이 존재할 경우, 각 손에 대해 21개의 랜드마크 좌표를 계산합니다.

> **image.flags.writeable = False**
> - 이미지 데이터를 읽기 전용으로 설정하여 MediaPipe 내부 처리 성능을 향상시킵니다.

> **results.multi_hand_landmarks**
> - 인식된 손이 하나 이상 존재할 경우, 각 손의 랜드마크 좌표가 리스트 형태로 저장됩니다. 손이 인식되지 않으면 None 값이 됩니다.

> **results.multi_handedness**
> - 각 손이 왼손인지, 오른손인지에 대한 분류 결과가 저장됩니다. landmark 정보와 같은 순서로 매칭됩니다.

> **draw_landmarks()**
> - 손의 각 관절 위치와 관절 간 연결선을 영상 위에 시각적으로 표시합니다. 인식 결과를 눈으로 확인하고 디버깅하기 위한 용도입니다.

> **count_fingers()**
> - 손 랜드마크 좌표를 기반으로 각 손가락이 펴져 있는지/접혀 있는지를 판단합니다. 엄지는 좌우 방향(x좌표), 나머지 손가락은 위아래 방향(y좌표)을 기준으로 비교합니다.

> **classify_gesture()**
> - 펴진 손가락 개수를 기준으로 주먹, 숫자 제스처 등의 이름을 결정합니다.제스처 인식의 가장 기본적인 규칙 기반 분류 방식입니다.
...
> - PC에서 전송된 데이터를 한 줄 단위(개행 문자 기준)로 읽어옵니다. 각도 값이 문자열 형태로 전송되므로 문자열 처리 방식이 적합합니다.

> **line.trim()**
> - 문자열 앞뒤에 포함될 수 있는 공백 문자나 개행 문자를 제거합니다.

> **toInt()**
> - 문자열로 전달된 각도 값을 정수형으로 변환합니다. 숫자가 아닌 문자열이 포함된 경우 0으로 변환됩니다.

> **constrain()**
> - 서보 모터가 물리적으로 회전할 수 있는 범위(0~180도)를 벗어나지 않도록 값을 제한합니다.

> **Servo.write()**
> - 전달받은 각도 값에 따라 서보 모터를 해당 위치로 회전시킵니다. 실제 하드웨어 제어는 아두이노가 담당하며, PC는 각도 계산만 수행합니다.
...
#### PC 프로그램 주요 코드 설명
> **serial.Serial()**
> - PC와 아두이노 간의 시리얼 통신을 초기화합니다. baudrate 값은 아두이노의 Serial.begin() 설정과 반드시 일치해야 합니다.

> **send_angle(current_angle)**
> - 현재 계산된 서보 각도를 문자열 형태로 변환하여 아두이노로 전송합니다.

> **if has_left and has_right:**
> - 왼손과 오른손이 동시에 인식되었는지를 확인합니다. 양손이 일정 시간 이상 유지될 경우 서보 모터를 중앙 위치로 복귀시키는 조건에 사용됩니다.

> **elif has_right:**
> - 오른손만 인식된 경우, 일정 주기마다 서보 각도를 증가시키는 상태로 판단합니다.

> **elif has_left:**
> - 왼손만 인식된 경우, 일정 주기마다 서보 각도를 감소시키는 상태로 판단합니다.

> **if (now - last_step_time) >= STEP_INTERVAL:**
> - 서보 각도가 너무 빠르게 변하지 않도록 시간 간격을 두고 제어합니다. 제스처 인식 결과를 부드러운 모터 움직임으로 변환하는 핵심 로직입니다.

> **cv2.putText(image, f"STATE: {state_text}", ...)**
> - 현재 손 인식 상태(LEFT / RIGHT / BOTH / NONE)와 서보 각도를 화면에 표시합니다. 사용자에게 현재 제어 상태를 직관적으로 전달합니다.