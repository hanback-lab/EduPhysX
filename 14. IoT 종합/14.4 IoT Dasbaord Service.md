...
Template에 필요한 설정은 Template 명칭, 하드웨어, 연결방식 입니다. 명칭은 임의로 입력해도 무방하며 하드웨어는 Arduino를 그리고 연결 방식은 Wi-Fi를 선택합니다. 
...
Dashboard 편집화면으로 진입하면 좌측에 활용 가능한 위젯의 종류가 나타나며, 우측에 대쉬보드에 원하는 위젯을 위치시켜 활용하는 형태입니다. 위젯은 무료로 활용 가능한 위젯과 유로로 활용 가능한 위젯으로 나뉘어 있습니다. 단순한 정수 데이터 표기 용도로 "Label" 위젯을 Dashboard에 드래그하여 원하는 위치에 위치합니다. 크기도 원하는 형태로 변환합니다. 배치한 위젯에 마우스 커서를 가져가면, 우측 상단에 3개의 메뉴가 출력되며 이 메뉴는 위젯을 복사, 위젯의 세부 설정, 위젯 삭제 기능을 담당합니다. 

배치한 "Label" 위젯에 앞서 생성한 Datastream 을 연결하기 위해 설정 화면으로 진입합니다. 
...
Label 위젯과 유사하게 LED 제어를 위한 Switch 위젯도 배치해보겠습니다. 

![Blynk Web Dashboard4](res/blynk_web_dashboard4.png)

![Blynk Web Dashboard5](res/blynk_web_dashboard5.png)
...
<img src="res/iot_dash_pinmap.png" style="width:80%">

### 장치 프로그래밍 
Blynk는 Arduino 및 Python 등의 라이브러리를 제공하며, REST API 및 MQTT 를 통한 다양한 접근 방식을 제공합니다. 상세한 안내는 다음 링크를 확인하시기 바랍니다. 
...
char broker[] = "blynk.cloud";
char authToken[] = "YOUR_TOKEN";
...
void setup() {
  ...
  if (client.connect("wizfi360-mega", "device", authToken)) {
    Serial.println("Connected to Blynk MQTT");
    client.onMessage(messageReceived);
    client.subscribe("downlink/ds/#");
  } else {
    Serial.println("Blynk MQTT connect failed");
  }
}
...
void loop() {
  ...
  if (millis() - lastMillis > 1000) {
    ...
    Serial.print("Publish A0 -> ");
    Serial.println(payload);

    client.publish("ds/Humi", payload);
  }
}
...
#### 주요 코드 설명
> **WizFi360 / WizFi360Mqtt 라이브러리**
> - WizFi360 Wi-Fi 모듈을 사용한 네트워크 연결과 MQTT 통신을 지원하는 라이브러리입니다. Wi-Fi 초기화, 브로커 접속, 토픽 발행 및 구독 기능을 제공합니다.

> **ssid / pass**
> - 무선 네트워크 접속에 필요한 SSID와 비밀번호를 저장합니다. 실제 환경에 맞게 수정해야 정상적인 연결이 가능합니다.

> **broker / authToken**
> - Blynk Cloud에서 제공하는 MQTT 브로커 주소와 장치 인증 토큰입니다. authToken은 Device 등록 시 발급된 고유 값으로, 장치 인증에 사용됩니다.

> **WiFiClient / MQTTClient 객체**
> - WiFiClient는 TCP 통신을 담당하고, MQTTClient는 이를 기반으로 MQTT 메시지 송수신을 수행합니다.

> **Serial / Serial1 초기화**
> - Serial은 디버깅 메시지 출력을 위한 포트입니다. Serial1은 WizFi360 Wi-Fi 모듈과의 UART 통신에 사용됩니다.

> **WiFi.init(&Serial1)**
> - WizFi360 모듈을 Serial1 포트에 연결하여 초기화합니다.

> **Wi-Fi 연결 대기 루프**
> - 무선 네트워크에 연결될 때까지 반복적으로 접속을 시도합니다. 연결이 완료되기 전까지 다음 단계로 진행하지 않습니다.

> **client.begin(broker, net)**
> - MQTT 브로커 주소와 네트워크 객체를 설정합니다.

> **client.connect(...)**
> - Blynk Cloud MQTT 브로커에 접속합니다. 이때 authToken을 이용해 장치를 인증합니다.

> **client.subscribe("downlink/ds/#")**
> - Dashboard에서 내려오는 모든 Datastream 제어 메시지를 수신하기 위해 downlink/ds/ 하위 토픽을 구독합니다.

> **client.loop()**
> - MQTT 통신을 유지하고 수신 이벤트를 처리하는 함수입니다. 반복적으로 호출되어야 메시지 송수신이 정상적으로 이루어집니다.

> **millis() 기반 주기 제어**
> - 1초마다 센서 값을 발행하도록 구성되어 있으며, delay()를 사용하지 않아 비동기 처리에 적합한 구조입니다.

> **analogRead(A0)**
> - 습도 센서가 연결된 A0 핀의 아날로그 값을 읽습니다.

> **client.publish("ds/Humi", payload)**
> - 센서 값을 문자열로 변환하여 Blynk Datastream에 대응되는 토픽으로 발행합니다.

> **messageReceived()**
> - MQTT 브로커로부터 메시지를 수신했을 때 자동으로 호출되는 콜백 함수입니다. 수신된 토픽과 데이터(payload)를 확인할 수 있습니다.

> **downlink/ds/LED 토픽 처리**
> - Dashboard의 Switch 위젯 조작 결과가 전달되는 토픽입니다. 전달된 값에 따라 LED를 ON/OFF 제어합니다.

토큰 정보등을 작성하여 실행하면 1초마다 A0의 값을 읽어 Blynk 에서 제공하는 브로커에 토픽을 발행합니다. 발행하는 토픽은 "ds/Humi" 입니다. 주기적으로 발행하고 있는 상황에 웹에서 Switch 를 클릭하여 제어하면 "downlink/ds/LED" 토픽이 발행되며 이를 아두이노에서 수신하면 토픽과 함께 전달된 payload 에 따라 LED 를 제어합니다. 

![Blynk Web Dashboard6](res/blynk_web_dashboard6.png)

정상적인 연결이 안되는 경우 코드에 작성한 Wi-Fi 연결정보, Token 정보가 틀리지 않았는지 다시 한번 확인합니다. 또한 Datastream 에 작성한 명칭이 코드토픽과 차이가 생긴다면 데이터 송수신이 되지 않으니 주의합니다. 
...
이후 Dashboard 구성 및 설정은 앞서 작업과 유사하며, 웹 환경에서 모니터링 및 제어하던 내용을 자신의 스마트폰에서도 확인이 가능해집니다. 
