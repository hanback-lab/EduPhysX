...
#### 토픽 생성 및 제거 
발행자는 메시지를 발행할 때 토픽 문자열을 함께 전달하며, 브로커는 해당 토픽 문자열을 기준으로 메시지를 라우팅합니다. 구독자 역시 토픽 문자열을 기준으로 메시지를 수신합니다. MQTT에서 토픽은 명시적으로 생성·삭제되는 개체가 아니라, 메시지 전달을 위한 계층적 문자열 구조이며, 세션이 종료되면 구독 정보만 제거됩니다.
...
#### 발행 
클린 세션으로 클라이언트를 연결하면 이전 세션에서 유지되던 구독 정보 및 미확인 메시지는 모두 제거됩니다. 이로 인해 클라이언트가 오프라인 상태일 때 발행된 QoS = 1 또는 QoS = 2 메시지는 이후 재연결 시 수신되지 않을 수 있습니다.
...
#### simple_pub.py 주요 코드 설명
> **client = mqtt.Client()**
> - MQTT 브로커와 통신하기 위한 클라이언트 객체를 생성합니다.

> **client.connect("127.0.0.1")**
> - 로컬 호스트(127.0.0.1)에 실행 중인 MQTT 브로커에 연결합니다. 이 예제에서는 익명 접속이 허용된 브로커를 사용하므로 별도의 인증 정보는 필요하지 않습니다.

> **client.loop_start()**
> - MQTT 네트워크 루프를 백그라운드 스레드로 실행합니다. 

> **client.publish("iot/topic/test", number)**
> - "iot/topic/test" 토픽으로 number 값을 발행합니다. 브로커는 해당 토픽을 구독 중인 모든 클라이언트에게 메시지를 전달합니다.
...
여기서 접속을 시도하는 "127.0.0.1" 즉, PC의 브로커는 익명 접속을 허용하는 브로커입니다. 따라서 별다른 사용자 정보를 입력하지 않아도 됩니다. 
...
#### simple_sub.py 주요 코드 설명
> **client.subscribe("iot/topic/test")**
> - "iot/topic/test" 토픽을 구독합니다. 해당 토픽으로 메시지가 발행되면 자동으로 수신하게 됩니다.

> **def on_message(...)**
> - 구독한 토픽에 메시지가 도착할 때마다 호출되는 콜백 함수입니다. message.topic은 토픽 이름, message.payload는 실제 데이터입니다.

> **client.loop_forever()**
> - MQTT 네트워크 루프를 메인 스레드에서 무한 실행합니다. 프로그램은 종료되지 않고 계속해서 메시지를 수신합니다.
...
#### bidirectional_client_a.py 주요 코드 설명
> **client.subscribe("iot/topic/interval")**
> - 메시지 발행 주기를 제어하기 위한 토픽을 구독합니다.

> **interval = int(message.payload)**
> - 수신된 메시지를 정수로 변환하여 발행 주기(interval)에 반영합니다.

> **client.publish("iot/topic/value", random.randrange(-20,60))**
> - -20 ~ 60 범위의 임의 값을 생성하여 센서 데이터처럼 발행합니다.

> **time.sleep(interval)**
> - 구독을 통해 전달받은 값에 따라 발행 주기가 동적으로 변경됩니다.
...
#### bidirectional_client_b.py 주요 코드 설명
> **input("Input Interval 1~10:")**
> - 사용자로부터 메시지 발행 주기를 입력받습니다.

> **client.publish("iot/topic/interval", interval)**
> - 입력된 값을 토픽으로 발행하여 다른 클라이언트의 동작을 제어합니다.

> **비동기 동작 구조**
> - 메시지 수신(on_message)과 사용자 입력 처리(main 루프)가 동시에 동작합니다.
...
#### cb_client_connect_a.py 주요 코드 설명
> **signal.signal(signal.SIGINT, signal_handler)**
> - Ctrl+C 입력을 감지하여 안전하게 브로커 연결을 종료합니다.

> **client.reconnect_delay_set(5)**
> - 재접속 시도 간격을 5초로 설정합니다.

> **on_disconnect() → reconnect()**
> - 네트워크 장애 발생 시 자동으로 브로커에 재연결합니다.
...
#### cb_client_connect_b.py 주요 코드 설명
> **on_connect() 내부에서 publish 수행**
> - 브로커에 연결된 직후, 발행 작업을 수행하도록 구성되어 있습니다. 연결이 보장된 상태에서만 메시지를 발행하기 위한 구조입니다.

> **client.publish("iot/sensor/value", i)**
> - 센서 값처럼 보이는 데이터를 동일한 토픽으로 반복 발행합니다.

> **client.disconnect()**
> - 모든 발행이 완료된 후 브로커와의 연결을 정상적으로 종료합니다.

> **on_disconnect()**
> - 연결 종료 시 호출되며, 정상 종료 여부를 확인하는 용도로 활용됩니다.
...
def publish_light(client):
    global qos 
    client.publish("iot/light/value", int(random.randint(0,100)), qos)
    time.sleep(1)
...
#### cb_client_pub.py 주요 코드 설명
> **on_publish(client, userdata, mid)**
> - publish() 호출 후 브로커로부터 응답을 수신했을 때 호출됩니다. QoS 1, QoS 2 환경에서 메시지 전달 여부를 확인하는 데 중요합니다.

> **QoS 단계적 증가 구조**
> - 동일한 메시지를 QoS 0 → 1 → 2 단계로 순차 테스트합니다. QoS 에 따른 동작 차이를 확인할 수 있습니다.

> **client.disconnect()**
> - 모든 QoS 테스트가 완료되면 브로커 연결을 종료합니다.
...
#### cb_client_sub.py 주요 코드 설명
> **client.subscribe("iot/light/value")**
> - 조도 또는 조명 상태와 같은 값을 수신하기 위한 토픽을 구독합니다.

> **value < 50 조건 처리**
> - 수신된 값이 특정 조건을 만족하면 브로커 연결을 종료합니다. 메시지 수신에 따른 조건 기반 제어 예제입니다.

> **비동기 메시지 처리 구조**
> - 메시지는 on_message() 콜백을 통해 즉시 처리되며
메인 루프의 흐름과 독립적으로 동작합니다.
...
def publish_light(client):
    tphg_data = {"temperature" : random.randint(0,100), "humidity" : random.randint(0,100)}
    light_data = random.randint(0,100)
    data = {
...
#### json_pub.py 주요 코드 설명
> **data = {...}**
> - 여러 센서 데이터를 하나의 딕셔너리 구조로 구성합니다.

> **json.dumps(data)**
> - 딕셔너리를 JSON 문자열로 변환하여 네트워크로 전송합니다.

> **client.publish("iot/sensor/value", json_data)**
> - 복합 데이터를 하나의 토픽으로 발행합니다.
...
#### json_sub.py 주요 코드 설명
> **json.loads(message.payload)**
> - 수신된 JSON 문자열을 다시 딕셔너리 형태로 복원합니다.

> **data['timestamp'], data['humi']**
> - JSON 데이터 중 필요한 항목만 선택적으로 사용합니다.